    ...
    for (let i = 0; i < options.driftCount; i += 1) {
      ...
      if (driftKind === 'alias-drift') {
        ...
        const aliasLineIndexes = lines
          ...
          .filter(({ line }) => line.trim().startsWith('- ') && !line.includes('screen'))
          .map(({ idx }) => idx);
        ...
        if (aliasLineIndexes.length > 0) {
          ...
          lines.splice(targetIdx + 1, 0, `${indent}- ${mutated}`);
          yield* fs.writeText(fullPath, lines.join('\n'));
        ...
        if (!content.includes(`${newElement}:`)) {
          ...
          modifiedFiles.push(`${hintsDir}/${screenId}.hints.yaml`);
