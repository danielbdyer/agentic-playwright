...
} from '../lib/domain/types/pipeline-config';
...
test('bottleneck weights sum to 1.0', () => {
  const sum = weightSum(DEFAULT_PIPELINE_CONFIG.bottleneckWeights);
  expect(Math.abs(sum - 1.0)).toBeLessThan(TOLERANCE);
});

test('proposal ranking weights sum to 1.0', () => {
  const sum = weightSum(DEFAULT_PIPELINE_CONFIG.proposalRankingWeights);
  expect(Math.abs(sum - 1.0)).toBeLessThan(TOLERANCE);
});

test('DOM scoring weights sum to 1.0', () => {
  const sum = weightSum(DEFAULT_PIPELINE_CONFIG.domScoringWeights);
  expect(Math.abs(sum - 1.0)).toBeLessThan(TOLERANCE);
});
...
test('merge with empty overrides preserves all weight sums', () => {
  ...
  expect(Math.abs(weightSum(merged.bottleneckWeights) - 1.0)).toBeLessThan(TOLERANCE);
  expect(Math.abs(weightSum(merged.proposalRankingWeights) - 1.0)).toBeLessThan(TOLERANCE);
  expect(Math.abs(weightSum(merged.domScoringWeights) - 1.0)).toBeLessThan(TOLERANCE);
});
...
test('calibrated weights preserve sum = 1.0 with positive correlations', () => {
  ...
  expect(Math.abs(weightSum(calibrated) - 1.0)).toBeLessThan(TOLERANCE);
});
