...
describe('Discovery-fitness visitor registry laws', () => {
  ...
  test('Law 3: DISCOVERY_VISITORS has exactly as many entries as DISCOVERY_FITNESS_METRIC_KINDS', () => {
    const visitorKeys = Object.keys(DISCOVERY_VISITORS);
    expect(visitorKeys.length).toBe(DISCOVERY_FITNESS_METRIC_KINDS.length);
  });

  test('Law 4: buildDiscoveryMetricTree returns a tree with one child per kind', () => {
    const tree = buildDiscoveryMetricTree(makeStubInput());
    expect(tree.metric.kind).toBe('discovery-root');
  ...
  test('Law 5: stub visitors return zero-value proxy metrics', () => {
    const input = makeStubInput();
    for (const kind of DISCOVERY_FITNESS_METRIC_KINDS) {
      const node = DISCOVERY_VISITORS[kind].visit(input);
