...
} from '../../product/reasoning/agent-interpreter-provider';
import { createScopedInternalMCPBridge } from '../../product/runtime/resolution/mcp-bridge';
  ...
  test('scoped session providers release clients across repeated runs', async () => {
    ...
    await Effect.runPromise(Effect.all(
      Array.from({ length: 20 }, () => Effect.scoped(Effect.gen(function* () {
        ...
        const response = yield* provider.interpret({} as never);
