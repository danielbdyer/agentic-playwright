...
import { classifySnapshotDrift } from '../lib/domain/drift';
...
test('classifySnapshotDrift detects each supported drift category', () => {
  const baseline = [
    'role: root',
    'children:',
    '  - role: button',
  ...
  const drift = classifySnapshotDrift(baseline, current);
  expect(drift.classes).toEqual([
    'accessible-name-change',
    'addition-removal',
    'role-change',
    'structural-move-order-change',
  ]);
  expect(drift.hasDrift).toBeTruthy();
});



test('classifySnapshotDrift detects structural move or order changes', () => {
  const baseline = [
    'role: root',
    'children:',
    '  - role: button',
  ...
  const drift = classifySnapshotDrift(baseline, current);
  expect(drift.classes).toContain('structural-move-order-change');
});

test('classifySnapshotDrift serialization is deterministic for equal inputs', () => {
  const baseline = [
    'role: root',
    'children:',
    '  - role: button',
  ...
  const first = classifySnapshotDrift(baseline, current);
  const second = classifySnapshotDrift(baseline, current);

  expect(JSON.stringify(first)).toBe(JSON.stringify(second));
  expect(first.driftFingerprint).toBe(second.driftFingerprint);
});
