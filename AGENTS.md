...
**Start here**: [`docs/v2-cold-start-cohort-brief.md`](docs/v2-cold-start-cohort-brief.md) (one page, plain English), then [`docs/v2-cold-start-todomvc-journal.md`](docs/v2-cold-start-todomvc-journal.md) (the experiment journal; read the last three entries first). The cohort machinery lives in `workshop/customer-backlog/` (`public-aut-runner.ts` + `public-aut/` fixtures + `cohort.json` manifest). The self-improvement praxis was hardened in cycle 11 (journal Entries 38–39): the empirical cohort now feeds the compounding engine (G1), has a committed regression ratchet (`--check-baseline`, G2), capture-and-replay (`--capture`, G3), honest DOM-target denominators + `--trials` (G4), a mechanical clean-room (`--evaluation-handoff` + contact ledger, G5), a calibrated resolver kernel (`workshop/optimization/`, G6), the product's own pattern registry running live as the unified `structured-pattern` rung (G7, `surface-harvest.ts`), and a file-mediated reasoning rung that bridges zero-overlap semantic cases like 日本語 (Z11d, `--reasoning-mode record|replay` + `reasoning-rung.ts` + `product/domain/reasoning-pool/` + `scripts/reasoning-fill.ts`). See [`docs/v2-self-improvement-praxis-audit.md`](docs/v2-self-improvement-praxis-audit.md) for the gap list G1–G10 + Z11d and its implementation-status table; the only deferred remainder is Z11d's operational autotelic-fill layer (live-adapter plan §7).
