...
**Start here**: [`docs/v2-cold-start-cohort-brief.md`](docs/v2-cold-start-cohort-brief.md) (one page, plain English), then [`docs/v2-cold-start-todomvc-journal.md`](docs/v2-cold-start-todomvc-journal.md) (the experiment journal; read the last three entries first). The cohort machinery lives in `workshop/customer-backlog/` (`public-aut-runner.ts` + `public-aut/` fixtures + `cohort.json` manifest). The self-improvement praxis was hardened in cycle 11 (journal Entry 38): the empirical cohort now feeds the compounding engine (G1), has a committed regression ratchet (`--check-baseline`, G2), capture-and-replay (`--capture`, G3), honest DOM-target denominators + `--trials` (G4), a mechanical clean-room (`--evaluation-handoff` + contact ledger, G5), and a calibrated resolver kernel (`workshop/optimization/`, G6). See [`docs/v2-self-improvement-praxis-audit.md`](docs/v2-self-improvement-praxis-audit.md) for the gap list G1–G10, its implementation-status table, and the deferred remainder (G7 unify-the-apparatus, G10 reasoning-metering-with-Z11d).
