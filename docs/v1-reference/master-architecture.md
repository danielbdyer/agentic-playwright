...
- **Source** — *which slot of the 6-rung precedence ladder produced this?* — `operator-override → agentic-override → deterministic-observation → reference-canon → live-derivation → cold-derivation`. Declared via the `Src` parameter on `Atom<C, T, Src>`, `Composition<S, T, Src>`, `Projection<S, Src>` in `lib/domain/pipeline/{atom,composition,projection}.ts`. No default parameter — every call site declares source explicitly. The `reference-canon` slot is transitional (2026-04-10 reframe — see `docs/canon-and-derivation.md` §§ 3.2a, 6); it carries pre-gate committed content that is clearly distinguished from promoted canonical artifacts and shrinks over time as real receipts and discovery promotions supplant it. The epistemic axis.
