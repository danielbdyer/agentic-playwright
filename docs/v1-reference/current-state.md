...
| Synthetic feature completion — Commit 1a: Source axis type extension | `implemented` (commit `87c73ff`) | `PhaseOutputSource` union extended from 5 to 6 variants with `'reference-canon'` at slot 4; `foldPhaseOutputSource` exhaustive; lookup chain gains `skipReferenceCanon?: boolean` flag orthogonal to `LookupMode`; 42 source-classifier tests pass. No loader wiring yet — Commit 1b closes that gap. See [`docs/synthetic-feature-completion-plan.md`](./synthetic-feature-completion-plan.md#commit-1a-summary-landed-2026-04-11). |
| Synthetic feature completion — Commit 1b: Reference-canon loader + catalog wiring + score report bucket | `planned` | Write a loader that walks `dogfood/knowledge/**`, `dogfood/benchmarks/**`, and non-intent `dogfood/controls/**`, invokes the existing per-class decomposers, and produces `ArtifactEnvelope<Atom<..., 'reference-canon'>>` entries merged into `WorkspaceCatalog.tier1Atoms` / `tier2Compositions`. Add a "reference-canon hit fraction" bucket to the score command's rung distribution report. |
