...
| T3.6.3 | **Scoring as a Contravariant Functor** — `contramapScoringRule` already exists but isn't recognized as implementing `Contravariant`. Formalize: `ScoringRule<T>` is `Contravariant` in `T`. `contramap(rule, f)` lets you reuse a `ScoringRule<Screen>` as a `ScoringRule<Scenario>` by projecting `scenario.screen`. This means scoring rules compose *across domain boundaries* without adaptation code | S | `contramapScoringRule` in `learning-shared.ts`, `ScoringRule<T>` | Scoring rules become universally reusable via contramap |
...
| T3.6.5 | **Governance as a Bounded Lattice with Galois Connection to Trust** — V1/W5 names the lattice. V3 goes further: the trust policy evaluation is the *lower adjoint* of a Galois connection between the `Governance` lattice and the `TrustDecision` lattice. Formalizing this means proving that relaxing trust policy thresholds monotonically relaxes governance gates — a safety property the system needs but doesn't prove | M | `GovernanceDesignation`, trust policy, `foldGovernance` | Safety property proven: loosening thresholds never accidentally tightens gates |
...
| T3.6.7 | **Convergence Detection as a Lyapunov Function** — The dogfood loop's convergence check is an unnamed Lyapunov function: a scalar `Φ(state)` that decreases monotonically toward zero. Naming it lets you prove termination bounds: "given current Φ slope, convergence will occur within N iterations." Currently convergence is detected empirically ("improvement dropped below threshold") without prediction | M | `ConvergenceMetrics` in dogfood loop, scorecard pass-rate trend | The system predicts when it will converge, not just whether it has |
